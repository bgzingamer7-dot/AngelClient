...
import io.github.solclient.client.ui.screen.cosmetics.CosmeticsScreen;
	...
	private static class MainMenuComponent extends Component {
		...
		public MainMenuComponent() {
			...
			add(new ButtonComponent((component, defaultText) -> I18n.translate("menu.options"), Theme.button(),
					Theme.fg()).withIcon("options").width(200).onClick((info, button) -> {
						if (button == 0) {
							MinecraftUtils.playClickSound(true);
...
add(
    new ButtonComponent(
        (component, defaultText) -> "",
        Theme.button(),                 // não é danger
        Theme.fg()
    )
    .withIcon("cosmetics")             // nome do ícone
    .width(20)
    .onClick((info, button) -> {
        if (button == 0) {
            MinecraftUtils.playClickSound(true);
            mc.setScreen(new CosmeticsScreen()); // ABRE A TELA
            return true;
        }
        return false;
    }),
    (component, defaultBounds) -> new Rectangle(
        screen.width - 52,              // um pouco à esquerda do exit
        6,
        defaultBounds.getWidth(),
        defaultBounds.getHeight()
    )
		...
		public void render(ComponentRenderInfo info) {
			...
			String copyrightString = "Sigam Jesus!!!";
			...
			String versionString = "AngelClient 1.8.9";
